...
**Active work:** the bounded three-fixture Gmail batch is deployed at `ce8e001`. Automated recovery and disconnect checks passed; live multi-message proof awaits three new synthetic Inbox messages.

**Latest verified result:** 136 backend tests and 23 browser tests passed, along with TypeScript, compilation and formatting checks. The deployed readiness check passes; the batch is empty and the earlier single fixture remains released at repeat revision 3. [Batch evidence](docs/evidence/phase-0/2026-09-05-controlled-batch.md).
...
Prepare three separate synthetic messages with subjects `phase0-batch-001`, `phase0-batch-002`, and `phase0-batch-003`, from the configured controlled sender to the allowed test account. Leave them in Inbox and unread. The read-only provider prerequisite check currently reports `fixture_mismatch`; no batch was created and no batch message writes occurred. Once the owner confirms the messages are ready, perform the authenticated live hold/recovery/release rehearsal and record provider evidence. The manual Gmail filter exercise is superseded and must not be repeated.
...
|---|---|---|
| 2026-09-05 | Fixed three-message Gmail batch deployed; per-message recovery and safe disconnect tested | [Evidence](docs/evidence/phase-0/2026-09-05-controlled-batch.md); 136 backend / 23 browser tests; live fixtures pending |

...
**Active work:** the held-batch safe-disconnect rehearsal passed. Remaining work focuses on live arrival/interception boundaries, independent monitoring and actual-device notification evidence.
...
The three-message batch is fully released at repeat revision one, and the test account is connected. Live hold, interrupted release, held-batch safe disconnect and interrupted revocation have passed within the controlled scope. Next prepare bounded arrival/interception proof, including fixed membership when a new message arrives during delivery. Concurrent-arrival membership remains covered by automated tests only. Independent alert receipt needs an owner-selected destination/service. The manual Gmail filter exercise is superseded and must not be repeated. Use the explicit revision-guarded repeat form for further rehearsals; do not reset durable records through administrative writes.
...
| 2026-09-05 | Fixed three-message Gmail batch deployed; per-message recovery and safe disconnect tested | [Evidence](docs/evidence/phase-0/2026-09-05-controlled-batch.md); 136 backend / 23 browser tests; subsequent live proof below |

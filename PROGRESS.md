...
**Active work:** live fixed three-message release crash recovery passed. Next proof gaps are live arrival/interception boundaries, batch disconnect and interruption during token revocation; independent alert setup and actual-device notification evidence remain outstanding.

**Latest verified result:** all three batch messages were held, then release was interrupted after Gmail accepted the second member. After web restart, recovery completed only the third write, with zero duplicates; repeated recovery made no further writes. All three are back in Inbox and unread, with no pending entries or errors. [Live batch evidence](docs/evidence/phase-0/2026-09-05-live-batch-recovery.md). The deployed slice previously passed 136 backend and 23 browser tests.

**Latest implemented slice:** fixed membership saved before Gmail writes, independently committed per-message results, read-before-write recovery, and restoration of all saved batch members before disconnect revokes access. See the [batch contract](docs/phase-0-controlled-batch.md). The earlier single-fixture live crash rehearsal recovered with zero duplicate writes; [crash evidence](docs/evidence/phase-0/2026-09-05-live-crash-recovery.md).

## Next action

The owner supplied the three batch fixtures and the live hold/interrupted-release/recovery rehearsal passed. The one-shot batch is now fully released; do not reset its durable record or re-hold it through administrative writes. Next, prepare the remaining bounded live proof for batch disconnect and revocation interruption, preserving explicit recovery intent and the existing released mail. Concurrent-arrival membership remains covered by automated tests only. Independent alert receipt still needs an owner-selected destination/service. The manual Gmail filter exercise is superseded and must not be repeated.

The owner uses the Gmail app on iPhone and reports all Gmail notifications, sounds and badges disabled. Batch notifications are optional and off by default by owner decision; this preference is documented, not yet an implemented settings feature. Independent alert setup, interruption during token revocation and the broader interception/device matrix remain open. The owner has authorized implementation, verification, pushing to main and deployment to the existing exe.dev VM. Group human Google instructions into one manageable batch.

...
The owner supplied the three batch fixtures and the live hold/interrupted-release/recovery rehearsal passed. The one-shot batch is now fully released; do not reset its durable record or re-hold it through administrative writes. Interrupted revocation, cleanup and reconnection now passed. Next, prepare the bounded live proof for disconnect from a held batch, preserving explicit recovery intent and the existing released mail. Concurrent-arrival membership remains covered by automated tests only. Independent alert receipt still needs an owner-selected destination/service. The manual Gmail filter exercise is superseded and must not be repeated.
...
- [ ] **Safe disconnect:** the [controlled flow](docs/phase-0-safe-disconnect.md) implements restore/verify before revoke with durable retries; live restoration from held, accepted revocation, cleanup and reconnect passed for the single fixture; single-message hold/release crash recovery passed; accepted-revocation request crash/restart/retry now passed without further mail operations; held-batch restoration and future general interception recovery remain unproven. Browser sign-out remains separate.

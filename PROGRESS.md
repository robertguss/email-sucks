...
Last updated: 2026-09-05 UTC (arrival rehearsal at 11:51 UTC)
...
**Current status:** work continues in this app; SSH and the correct Gmail incognito session were restored. Concurrent-arrival rehearsal is in progress at batch repeat revision two. At 11:51:35 UTC, release was interrupted after the second accepted provider write: the database has one released member and two pending, while Gmail has the first two in Inbox and the third still held. All three remained unread. Diagnostic removal and web restart completed; at 11:52 UTC readiness passed and the normal adapter was restored. The new subject had zero matching messages before the owner send request. Awaiting the new arrival. Do not repeat hold or reset the batch; resume the saved release only after the new fixture has arrived.
...
After confirming diagnostic cleanup and readiness, ask the owner to send one separate email with subject `phase0-arrival-001` from the configured sender to the receiving test account, body “test”, leaving it unread in Inbox. Verify its identity, arrival time and exclusion from the three saved IDs before recovery; then resume the saved release and verify the newcomer remains unchanged and outside membership. This live proof is not yet complete. Broader automatic interception, independent alert setup and device proof remain open. Do not reset durable records or repeat the superseded manual Gmail filter exercise.

...
for (const state of ['not_started', 'holding', 'held', 'releasing', 'released']) {
  test(`batch ${state} displays progress and submits only a CSRF-protected action`, async ({ page }) => {
    ...
    await page.route('http://127.0.0.1:4010/', route => {
      const props = { gmail_connected: true, gmail_configured: true, gmail_email: 'fixture@example.test', gmail_reconnect: false, gmail_checked: true, csrf_token: 'test-csrf', notice: null, batch: { state, total: 3, held: 1, released: 1, pending: 1, errors: 1 } };

...
for (const state of ['not_started', 'hold_pending', 'held', 'release_pending', 'released']) {
  test(`controlled actions reflect saved ${state} state and submit CSRF`, async ({ page }) => {
    ...
    await page.route('http://127.0.0.1:4010/', route => {
      const props = { gmail_connected: true, gmail_configured: true, gmail_email: 'fixture@example.test', gmail_reconnect: false, gmail_checked: true, csrf_token: 'fixture-csrf', notice: null, controlled: { state, repeat_revision: 7, verified_at: state === 'held' || state === 'released' ? 1788566400 : null } };
    ...
    if (state === 'released') await page.getByText('Repeat the recovery test', { exact: true }).click();
    ...
    if (state === 'released') expect(new URLSearchParams(submitted.postData()!).get('repeat_revision')).toBe('7');

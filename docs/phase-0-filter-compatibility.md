...
The current prototype cannot safely execute this test yet. Its recovery flow owns message labels and fixed membership, but no interception filters, filter-creation journal, filter drift check or disable-before-restore step. Build and test that bounded lifecycle before requesting activation. Required failure checks include response loss after filter creation, duplicate activation, wrong/stale ownership, partial filter removal, revoked settings access and disconnect while the test is active. Never delete a user filter based only on matching criteria or label names.
